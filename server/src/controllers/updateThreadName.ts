...
const updateThreadName = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const updateThreadTitle = await pool.query(`UPDATE threads SET name = $1::text WHERE id = $2::uuid RETURNING id;`, [
      threadName, threadId
    ]);
    if (updateThreadTitle.rows.length === 0) return utils.sendResponse(res, 503, "Failed to update thread name");
