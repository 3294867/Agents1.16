...
const PauseRunButton = ({ threadId, requestId, responseId, input, isNew, isDisabled, isEditing, setIsEditing, agentModel }: Props) => {
  ...
  return !isEditing
    ? (
      <button onClick={handlePause} className='h-9 w-9 text-text-button flex justify-center items-center rounded-full border border-border cursor-pointer transition-colors duration-200 bg-background hover:bg-black'>
      ...
      <button onClick={handleRun} className='h-9 w-9 text-text-button flex justify-center items-center rounded-full border border-border cursor-pointer transition-colors duration-200 bg-background hover:bg-black'>
